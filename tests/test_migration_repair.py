...
async def temp_db():
    source_url = dsn()
    assert source_url, "DATABASE_URL must point to an isolated PostgreSQL test database"
    source_name = source_url.rsplit('/', 1)[1].split('?', 1)[0]
    assert re.fullmatch(r'[A-Za-z0-9_]+', source_name)
    assert 'test' in source_name.lower(), "Migration repair refuses a non-test database"
...
async def test_migration_repair_idempotency(temp_db):
    ...
    # 1. Build the base state in the empty isolated database.
    await asyncio.to_thread(run_alembic_upgrade, temp_db, "1eadcdac923e")
