...
@pytest.mark.asyncio
async def test_goods_page_manual_products(make_callback_query, mock_fsm):
    from bot.handlers.user.shop_and_goods import _render_goods_page
    from aiogram.enums import ButtonStyle

    call = make_callback_query("dummy_cpage")
    page_items = [
        (101, "Manual Available", "manual"),
        (102, "Manual Unavailable", "manual"),
    ]
    ...
    stock_dict = {101: 0, 102: 0} # Manual items typically have 0 stock (no ItemValues)
    ...
    with patch('bot.handlers.user.shop_and_goods.LazyPaginator', return_value=mock_paginator), \
         ...
         patch('bot.handlers.user.shop_and_goods.get_store_settings', new_callable=AsyncMock) as mock_settings, \
         patch('bot.handlers.user.shop_and_goods.localize') as mock_localize, \
         patch('bot.handlers.user.shop_and_goods.EnvKeys') as mock_env_keys:

        mock_get_stock.return_value = stock_dict
        mock_settings.return_value.product_columns = 1
        ...
        # Test 1: Manual checkout enabled
        mock_env_keys.MANUAL_CHECKOUT_ENABLED = True
        await _render_goods_page(call, mock_fsm, category_id=1, page=0)
        ...
        assert buttons[0][0].text == "Manual Available · ♾️ Available"
        assert buttons[0][0].style == ButtonStyle.PRIMARY
        assert buttons[0][0].callback_data == "itm:0:0"

        assert buttons[1][0].text == "Manual Unavailable · ♾️ Available"
        assert buttons[1][0].style == ButtonStyle.PRIMARY
        assert buttons[1][0].callback_data == "itm:1:0"

        # Test 2: Manual checkout disabled
        mock_env_keys.MANUAL_CHECKOUT_ENABLED = False
        await _render_goods_page(call, mock_fsm, category_id=1, page=0)
        ...
        assert len(buttons) >= 2
        assert buttons[0][0].text == "Manual Available · ⛔ Sold Out"
        assert buttons[0][0].style == ButtonStyle.DANGER
        assert buttons[0][0].callback_data == "itm:0:0"

        assert buttons[1][0].text == "Manual Unavailable · ⛔ Sold Out"
        assert buttons[1][0].style == ButtonStyle.DANGER
        assert buttons[1][0].callback_data == "itm:1:0"
