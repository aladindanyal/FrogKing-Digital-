...
def compute_schema_fingerprint(fields: List[ProductCustomerField]) -> str:
    ...
    for f in sorted_fields:
        ...
        if f.field_type == 'select' and f.select_options_i18n:
            if isinstance(f.select_options_i18n, dict):
                option_keys.extend(f.select_options_i18n.keys())
            elif isinstance(f.select_options_i18n, list):
...
def validate_field_input(field: ProductCustomerField, value: str) -> str:
    ...
    if field.field_type == 'email':
        ...
        valid_keys = []
        if isinstance(field.select_options_i18n, dict):
            valid_keys = list(field.select_options_i18n.keys())
        elif isinstance(field.select_options_i18n, list):
